...
let check_unsupported ~is_reasoning (opts : Ai_provider.Call_options.t) =
  ...
  Stdlib.Option.iter (fun _ -> warn "top_k" (Some "OpenRouter does not support top_k")) opts.top_k;
...
let prepare_request ~model ~stream (opts : Ai_provider.Call_options.t) =
  ...
  let tools_json, tool_choice_json =
    ...
    | Object_tool { tool_name; schema = { name = _; schema } } ->
      ...
      let tools =
        List.map Convert_tools.openai_tool_to_json
          (Convert_tools.convert_tools ~strict:or_opts.strict_json_schema [ tool ])
      in
    ...
    | tools ->
      let tools_json =
        List.map Convert_tools.openai_tool_to_json
          (Convert_tools.convert_tools ~strict:or_opts.strict_json_schema tools)
      in
  ...
  let max_tokens, max_completion_tokens =
    match model_caps.is_reasoning_model, or_opts.max_completion_tokens, opts.max_output_tokens with
  ...
  let reasoning_effort =
    Stdlib.Option.map Openrouter_options.reasoning_effort_to_string or_opts.reasoning_effort
  in
  let stop =
  ...
  in
  let transforms =
    match or_opts.transforms with
    | [] -> None
    | ts -> Some ts
  in
  let route = Stdlib.Option.map Openrouter_options.route_preference_to_string or_opts.route in
  let provider = Stdlib.Option.map Openrouter_options.provider_prefs_to_json or_opts.provider_preferences in
  let api_keys =
    match or_opts.api_keys with
    | [] -> None
    | keys -> Some (Openrouter_options.api_keys_to_json keys)
  in
  let body =
    Openrouter_api.make_request_body ~model ~messages:messages_json ?temperature ?top_p ?max_tokens
      ?max_completion_tokens ?frequency_penalty ?presence_penalty ?stop ?seed:opts.seed ?response_format
      ?tools:tools_json ?tool_choice:tool_choice_json ?parallel_tool_calls:None ?reasoning_effort ?plugins ?transforms
      ?route ?provider ?api_keys ~include_reasoning:or_opts.include_reasoning ~stream ()
  in
  body, warnings
...
let create ~config ~model =
  let module M = struct
    ...
    let generate opts =
      ...
      let%lwt response = Openrouter_api.chat_completions ~config ~body ~extra_headers:opts.headers ~stream:false in
      match response with
      | `Json json ->
    ...
    let stream opts =
      ...
      let%lwt response = Openrouter_api.chat_completions ~config ~body ~extra_headers:opts.headers ~stream:true in
      match response with
      | `Stream line_stream ->
